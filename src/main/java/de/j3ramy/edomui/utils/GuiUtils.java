...
public class GuiUtils {
    ...
    public static String getTruncatedLabel(String s, float fontSizeScale, int maxTextWidth) {
        ...
        int scaledTextWidth = (int) (Minecraft.getInstance().font.width(s) * fontSizeScale);
        if (scaledTextWidth <= maxTextWidth) {
            return s;
        }

        int ellipsisWidth = (int) (Minecraft.getInstance().font.width("...") * fontSizeScale);
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            int charWidth = (int) (Minecraft.getInstance().font.width(currentChar) * fontSizeScale);
    ...
    private static Date parseTimestampOrDate(String timestamp) {
        ...
        // If parsing with time fails, try parsing without time and set time to 00:00:00
        ...
        // If nothing works, return current date as a fallback (or handle error as needed)
        return new Date();
    }
